...
def add_property(property_data: PropertySchema, session: SessionLocal):
...
def add_offeror(property_data: PropertySchema, session):
    exists_query = session.query(exists().where(
        Offeror.name == property_data.offeror_name,
        Offeror.type == property_data.offeror_type
    )).scalar()
    ...
    if exists_query:
        ...
        offeror_obj = Offeror(
            ...
            type=property_data.offeror_type,
...
def offer_exists(property_data: PropertySchema, session):
    exists_query = session.query(exists().where(
...
def get_offer(property_data: PropertySchema, session):
    return session.query(Offer).filter(
        Offer.inner_id == property_data.inner_id,
        Offer.source == property_data.source
    ).first()

def add_offer(property_data: PropertySchema, offeror_id, property_id, session):
...
def add_update(property_data: PropertySchema, offer_id, session):
...
def update_existing_offer(property_data: PropertySchema, session):
...
def handle_data(property_data: PropertySchema):
